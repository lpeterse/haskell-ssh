...
import           Control.Concurrent.Async       (Async, link, cancel, waitCatchSTM,
                                                 withAsync)
...
import           Control.Exception              (fromException, throwIO, catch)
...
serve config stream = withDisconnectHandler $ do
    ...
    where
        withDisconnectHandler run = action `catch` handler
            where
                action = run >>= (onDisconnect config . Right)
                handler e = onDisconnect config (Left e) >> throwIO e 

serveTransport :: Config identity -> Version -> Version -> TransportState -> IO Disconnect
serveTransport config clientVersion serverVersion state = do
    ...
    withAsyncSender config state $ \enqueue -> do
        ...
        (session, kexNextStep) <- performInitialKeyExchange config state enqueue clientVersion serverVersion
    ...
    where
        processIncomingMessages :: (KexStep -> IO ()) -> Connection identity -> IO Disconnect
...
withAsyncWatchdog config state rekey run =
    withAsync runWatchdog $ \thread -> link thread >> run
    where
...
withAsyncSender :: Config identity -> TransportState -> ((Message -> IO ()) -> IO a) -> IO a
withAsyncSender config state runWith = do
    ...
    let dequeue = atomically $ (MsgDisconnect <$> readTMVar outDisconnect) <|> readTChan outQueue
    withAsync (runSender dequeue) $ \thread -> link thread >> runWith enqueue `catch` \e -> do
