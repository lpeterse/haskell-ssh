...
chacha20poly1305 :: (BA.ByteArrayAccess key) => (BS.ByteString -> [key]) -> (EncryptionContext, DecryptionContext)
chacha20poly1305 deriveKeys = (encryptionContext, decryptionContext)
    where
