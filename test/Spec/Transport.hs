...
test01 = testCase "key exchange yields same session id on both sides" $ do
    ...
    where
        runServer stream config  = withTransport config stream getSessionId
        runClient stream config  = withTransport config stream getSessionId
...
test02 = testCase "server sends first message, client receives" $ do
    ...
    where
        ...
        runServer stream config  = withTransport config stream $ \transport -> do
            sendMessage transport (ChannelData (ChannelId 0) "ABCD")
            pure ()
        runClient stream config  = withTransport config stream $ \transport -> do
